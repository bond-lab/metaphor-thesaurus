...
strings with \texttt{embeddinggemma} (a 768-dimensional encoder served
locally via Ollama) and compute cosine similarity:
\begin{equation}
  \mathrm{sim}_{\mathrm{emb}}(q, d_s)
    = \frac{\mathbf{e}_q \cdot \mathbf{e}_s}
           {\|\mathbf{e}_q\|\,\|\mathbf{e}_s\|},
\end{equation}
where $\mathbf{e}_q$ and $\mathbf{e}_s$ are the respective embedding
vectors.
